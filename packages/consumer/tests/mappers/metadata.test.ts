...
import { MetadataMapper } from '../../src/mappers/metadata.ts';
import {
...
describe('MetadataMapper', () => {
	...
	describe('mapLastIndexedBridgeTx', () => {
		test('should map last indexed bridge transaction correctly', () => {
			const result = mapper.mapLastIndexedBridgeTx(mockLastIndexedBridgeTransaction);

			expect(result).toEqual({
				lastIndexedBridgeDepositCount: mockLastIndexedBridgeTransaction.deposit_count
			});
		});

		test('should map different deposit counts correctly', () => {
			const customBridgeTransaction = { deposit_count: 999 };
			const result = mapper.mapLastIndexedBridgeTx(customBridgeTransaction);
	...
	describe('mapLastIndexedClaimTx', () => {
		test('should map last indexed claim transaction correctly', () => {
			const result = mapper.mapLastIndexedClaimTx(mockLastIndexedClaimTransaction);

			expect(result).toEqual({
				lastIndexedClaimBlockNumber: mockLastIndexedClaimTransaction.block_num
			});
		});
	...
	describe('mapLastIndexedMappingTx', () => {
		test('should map last indexed mapping transaction correctly', () => {
			const result = mapper.mapLastIndexedMappingTx(mockLastIndexedMappingTransaction);

			expect(result).toEqual({
				lastIndexedMappingBlockNumber: mockLastIndexedMappingTransaction.block_num
			});
		});

		test('should map different block numbers correctly', () => {
			const customMappingTransaction = { block_num: 789012 };
			const result = mapper.mapLastIndexedMappingTx(customMappingTransaction);
		...
		test('should map zero block number correctly', () => {
			...
			const result = mapper.mapLastIndexedMappingTx(zeroMappingTransaction);
	...
	describe('return type consistency', () => {
		test('all mapping methods should return objects with single property', () => {
			const bridgeResult = mapper.mapLastIndexedBridgeTx(mockLastIndexedBridgeTransaction);
			const claimResult = mapper.mapLastIndexedClaimTx(mockLastIndexedClaimTransaction);
			const mappingResult = mapper.mapLastIndexedMappingTx(mockLastIndexedMappingTransaction);
			...
			expect(bridgeResult).toHaveProperty('lastIndexedBridgeDepositCount');
			expect(claimResult).toHaveProperty('lastIndexedClaimBlockNumber');
			expect(mappingResult).toHaveProperty('lastIndexedMappingBlockNumber');
		});
		...
		test('should handle large numbers correctly', () => {
			...
			expect(bridgeResult.lastIndexedBridgeDepositCount).toBe(largeNumber);
			expect(claimResult.lastIndexedClaimBlockNumber).toBe(largeNumber);
			expect(mappingResult.lastIndexedMappingBlockNumber).toBe(largeNumber);
		});
	});
});
