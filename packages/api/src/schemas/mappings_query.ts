...
export const MappingsQuerySchema = z
    .object({
        originTokenAddress: z.string().optional().transform(val => val?.toLowerCase()),
        wrappedTokenAddress: z.string().optional().transform(val => val?.toLowerCase()),
...
export const MappingsByTokenQuerySchema = z
    .object({
        ...
        tokenAddress: z.string().nonempty().optional().transform(val => val?.toLowerCase()),
    })
    .merge(NetworkSchema);
