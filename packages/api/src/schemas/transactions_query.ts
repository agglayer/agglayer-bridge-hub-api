...
export const TransactionsQuerySchema = z
    .object({
        ...
        updatedSince: z.coerce.number().int().nonnegative().refine(
            (val) => val === undefined || (val >= 1000000000000 && val <= 9999999999999),
            {
                message: "updatedSince must be a valid Unix timestamp in milliseconds (13 digits)"
            }
        ).optional(),
        status: z
