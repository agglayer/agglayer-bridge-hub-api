...
import { MappingsController } from "../controllers/mappings";
import {
...
const createMappingsRoutes = (mappingsService: MappingsService) => {
	...
	const getMappingsRoute = createRoute({
		...
		description:
			"Retrieve a paginated list of token mappings between networks",
		request: {
	...
	// Register routes
	mappingsRoutes.openapi(getMappingsRoute, mappingsController.getMappings);
	mappingsRoutes.openapi(
		getMappingsByTokenRoute,
		mappingsController.getMappingsByToken
	);

	return mappingsRoutes;
};

export default createMappingsRoutes;
