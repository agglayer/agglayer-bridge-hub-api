...
describe("TransactionService", () => {
	...
	describe("getProof", () => {
		test("should fetch and return valid proof", async () => {
			...
			const result = await service.getProof(
				sourceNetwork,
				depositCount,
				leafIndex
			);
	...
	describe("computeGlobalIndex", () => {
		test("should compute global index for mainnet (network 0)", () => {
			...
			const result = service.computeGlobalIndex(
				indexLocal,
				sourceNetworkId
			);
		...
		test("should produce different results for different networks", () => {
			...
			const network137Result = service.computeGlobalIndex(
				indexLocal,
				137
			);
		...
		test("should be deterministic for same inputs", () => {
			...
			const result1 = service.computeGlobalIndex(
				indexLocal,
				sourceNetworkId
			);
			const result2 = service.computeGlobalIndex(
				indexLocal,
				sourceNetworkId
			);
