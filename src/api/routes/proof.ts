...
export function setupProofApiRoutes(app: Elysia): Elysia {
  return app.get(
    ...
    async ({ query, set }) => {
      ...
      Logger.info(`Starting Proof API request for network=${networkId} & depositCount=${depositCount}`);

      try {
        const finalResult = await proofApiService.getProof(Number(depositCount), Number(networkId));
